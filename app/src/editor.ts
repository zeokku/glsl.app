...
import { glInitialization } from "./components/Canvas.vue";
...
/**
 * Compile shader and update line decorations
 * @param code
 * @returns
 */
export const compileShader = async (code: string) => {
  ...
  const infoLog = await updateFragment(code);

  updateLineDecorations?.(infoLog);
};
...
let model = editor.createModel("", "glsl");
...
export const init = (container: HTMLElement) => {
   ...
   */
  updateLineDecorations = (infoLog?: string | null) => {
    /*
  ...
  */
    ...
    infoLog &&
      infoLog
        .split("\n")
        // @note glsl logs are ended with \0 terminator
        .filter(s => /[^\s\0]/.test(s))
        .forEach(line => {
          ...
          if (logChunks.length !== 5) {
            ...
            markers.push({
              // @note bruh if message is empty string it will highlight line number but no code underline
              message: capitalize(rest.join(":").trim()),
          ...
          message = fragment.trim() + " | " + capitalize(message);
...

  {
    ...
    languages.registerCompletionItemProvider("glsl", {
      ...
      provideCompletionItems(model, position, context, token) {
        ...
        if (!match) {
          ...
          return {
            suggestions: [
              ...
              {
                label: "Shader Toy",
                // @note lol i have to escape $ sign
                insertText: "\\$shadertoy",
                documentation: "Include a list of defines to easily import code from Shader Toy",
                kind: CustomCompletionItemKind.Tools,
                insertTextRules: languages.CompletionItemInsertTextRule.InsertAsSnippet,
                range: collapsedRange,
                sortText: "b",
                additionalTextEdits: wordUnderCursor
              ...
              {
                label: "GLSL 1.0 Compatibility",
                insertText: "\\$glsl1",
                documentation: "Include a list of defines to easily import GLSL 1.0 code",
                kind: CustomCompletionItemKind.Tools,
                insertTextRules: languages.CompletionItemInsertTextRule.InsertAsSnippet,
                range: collapsedRange,
                sortText: "b",
        ...
        let obj = parsedLygia as Record<string, any | null>;
        ...
        if (obj) {
          ...
          items.forEach(i => {
            ...
            suggestions.push({
              ...
              // @ts-ignore incorrect types? why is title required?
              command: isDir
                ? {
                    id: "editor.action.triggerSuggest",
                  }
  ...
  languages.registerLinkProvider("glsl", {
    provideLinks(model, token) {
      ...
      for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
        ...
        if (!dep || dep.type !== "module" || dep.path[0] === "$") continue;
  ...
  /**
   * @todo tbh we need to use list for zones and lenses to avoid dupes problems
   * when we have invalid unresolved includes (null returned) it will have a codelens applied to the first invalid include which is nice
   */
  ...
  languages.registerCodeLensProvider("glsl", {
    ...
    provideCodeLenses(curModel, token) {
      ...
      let processedIncludes: string[] = [];

      // @note show loading code lens when includes are not loaded (we can't use includesData)
      curModel.getLinesContent().forEach((line, lineIndex) => {
        ...
        // @note skip updating lens range on encountered duplicated includes
        if (processedIncludes.includes(includePath)) return;
        ...
        // include is found, but not loaded yet
          ...
          let includeError = currentIncludesData.find(data => data[2] === includePath)?.[4];
          ...
          lenses.push({
            ...
            command: {
              id: "",
              title: includeError ? "⚠\uFE0E Error" : "⏳\uFE0E Loading...",
            },
          });
        }
      });
  ...
  //#region on model content change, ast processing + update zones
  {
    ...
    const onChangeModelContent = async () => {
      // @note this will trigger shader saving watcher
      currentShader.value!.code = model.getValue();

      let linesContent = model.getLinesContent();

      console.time("process-includes-start");

      let processedCode = await processIncludes(linesContent, currentIncludesData);

      console.timeEnd("process-includes-start");

      if (isManualRecompilation.value) {
        // @note we still can have includes errors so update them
      ...
      try {
        ...
        console.timeEnd("parsed-includes");

        console.time("parsed-model");

        // @note use worker
        editorAst = await parse(model.getValue());
        // editorAst = parse(model.getValue(), { includeLocation: true });
        // console.log(ast.scopes)

        console.timeEnd("parsed-model");
      ...
      editorInstance.changeViewZones(zonesAccessor => {
        ...
        console.log("changeViewZones", currentIncludesData, currentZones);

        currentIncludesData.forEach(([lineNumber, linesCount, includePath, code, error]) => {
          if (error) return;

          if (currentZones.has(includePath)) {
            // @note if we don't have duplicated include
            if (!newZones.has(includePath)) {
              let zone = currentZones.get(includePath)!;
        ...
        console.log("newZones", currentZones);
