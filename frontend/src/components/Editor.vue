...
import { languages, editor, MarkerSeverity, Range, KeyMod, KeyCode, Position, IMarkdownString, IPosition } from 'monaco-editor';
...
onMounted(async () => {
    ...
    let messageController = editorInstance.getContribution('editor.contrib.messageController') as MessageController;
        ...
        scope.run(() => {
            watch(() => props.infoLog, (infoLog) => {
                ...
                */
                ...
                for (let [includeLineNumber, includeLinesCount, /* id */, /* code */, failed] of currentIncludesData) {
                ...
                infoLog
                    ...
                    .forEach(line => {
                        ...
                        if (indexOfFragment === -1 || modelLine.indexOf(codeFragment, indexOfFragment + 1) !== -1) {
                        ...
                        message = fragment.trim() + ' | ' + message.slice(0, 1).toUpperCase() + message.slice(1)
                ...
                let lineDecorations = markers.map(m => ({
    ...
    interface VarConstDefinition extends BaseDefinition {
        ...
        kind: languages.CompletionItemKind.Variable | languages.CompletionItemKind.Constant | CustomCompletionItemKind.Misc,
    ...
    languages.registerCompletionItemProvider("glsl", {
        provideCompletionItems(curModel, position) {
            ...
            for (let def of dynamicDefinitionsData) {
                if (!def.range.containsPosition(position))
                    continue;
            ...
            let suggestions: languages.CompletionItem[] = [...staticSuggestions, ...dynamicSuggestions.values()];
            ...
            let matchRange = new Range(position.lineNumber, word.startColumn, position.lineNumber, word.endColumn)
    ...
    //#region property autocompletion
    {
        ...
        const vecFields = [['x', 'y', 'z', 'w'], ['s', 't', 'p', 'q'], ['r', 'g', 'b', 'a']];
        ...
        languages.registerCompletionItemProvider('glsl', {
            ...
            provideCompletionItems(model, position, context, token) {
                ...
                let line = model.getValueInRange(new Range(position.lineNumber, 1, position.lineNumber, position.column)).trim();
                ...
                let word = parts[0];
                let symbol = dynamicDefinitionsData.findLast(d => d.range.containsPosition(position) && d.name === word);
    ...
    const addSemanticTokenData = (loc: LocationInfo, length: number, type: SemanticTokenData['type'], modifier?: SemanticTokenData['modifier']) => {
...

    {
        ...
        const addSemanticTokenData = (
            ...
            buffer.push(lineOffset, colunOffset, length, tokenTypes.indexOf(token), tokenModifiers.includes(modifier) ? 1 << tokenModifiers.indexOf(modifier) : 0)
        }
        ...
        const insertLeft = <NodeValue extends LocationInfo = LocationInfo>(n: TreeNode<NodeValue>, v: NodeValue) => {
            if (n.l) {
                insert(n.l, v);
            }
            else {
        ...
        const insertRight = <NodeValue extends LocationInfo = LocationInfo>(n: TreeNode<NodeValue>, v: NodeValue) => {
            if (n.r) {
                insert(n.r, v)
            }
            else {
        ...
        const insert = <NodeValue extends LocationInfo = LocationInfo>(n: TreeNode<NodeValue>, v: NodeValue) => {
            if (v.line < n.v.line) {
                insertLeft(n, v);
            }
        ...
        const traverse = <NodeValue extends LocationInfo = LocationInfo>(n: TreeNode<NodeValue> | undefined, cb: (e: TreeNode<NodeValue>) => void) => {
            if (!n) return;
        ...
        provideHover(curModel, hoverPosition) {
                ...
                if (glslBuiltin) {
                    return {
                        ...
                        contents: [
                            ...
                            {
                                value: glslBuiltin.desc.replace(new RegExp(String.raw`\b${word}\b`), "**`" + word + "`**"),
                            },
                            {
            ...
            //#region bindings and types
            {
                ...
                for (let def of dynamicDefinitionsData) {
                    ...
                    if (!def.range.containsPosition(hoverPosition))
                        continue;
    ...
    //#region signature help provider
    {
        const colorize = (chunk: string) => {
            return editor.colorize(chunk, 'glsl-md', {})
                .then(html =>
        ...
        const createHybridFunctionHighlight = async (name: string, def: FunctionDefinition['overloads'][number], paramIndex: number) => {
            ...
            for (let i = 0; i < paramEntries.length; i += 1) {
                ...
                let paramStr = (paramQualifiers.length ? (paramQualifiers.join(' ') + ' ') : '') + paramType + ' ' + paramName;
        ...
        const createBuiltinHighlight = async (def: BuiltinSignatureInformation['def'], paramIndex: number) => {
        ...
        const updateArgumentNumber = async (signatures: SignatureInformation[], paramNumber: number) => {
        ...
        languages.registerSignatureHelpProvider('glsl', {
            ...
            async provideSignatureHelp(model, position, token, context) {
                ...
                let nextCharacter = model.getValueInRange(Range.fromPositions(position, position.delta(0, 1)))
                let prevCharacter = model.getValueInRange(Range.fromPositions(position.delta(0, -1), position))
                if (nextCharacter === '(' || prevCharacter === ')') return;

                console.log('prev sig', context.activeSignatureHelp)

                let lineContentBeforePosition = model.getValueInRange(
                    new Range(
                        position.lineNumber,
                        1,
                        position.lineNumber,
                ...
                if (context.activeSignatureHelp) {
                    ...
                    // @ts-ignore
                    await updateArgumentNumber(activeSignatureHelp.signatures, activeSignatureHelp.activeParameter)

                    //#region change selected signature to correspond the amount of arguments

                    let activeSignature = activeSignatureHelp.signatures[activeSignatureHelp.activeSignature] as SignatureInformation;
                    let paramsNum;
                    ...
                    if (activeSignatureHelp.activeParameter >= paramsNum) {
                        ...
                        let sigIndex = (activeSignatureHelp.signatures as SignatureInformation[]).findIndex(s => {
                            if ('builtin' in s) {
                        ...
                        for (let o of overloads) {
                            ...
                            for (let fullParam of params) {
                                ...
                                parameters.push({
                                    label: fullParam,
                                    documentation: {
                                        value: `<code>${await colorize(fullParam)}</code> - ${builtin.params![paramName]}`,
                                        supportHtml: true
                                    }
                                })
                            }
    ...
    // colors are only processed for vec3/vec4 when all values are in 0-1 range and declaration spans within one line (no line breaks)
    {
        ...
        const vecRegex = new RegExp(String.raw`\bvec[34] *\( *(${numberRegex} *, *${numberRegex} *, *${numberRegex}(?: *, *${numberRegex})?) *\)`, 'g');
        ...
        languages.registerColorProvider('glsl', {
            provideColorPresentations(curModel, { color, range }) {
                ...
                // -1 for index + 3 ("vec")
                let dim = parseFloat(model.getLineContent(range.startLineNumber)[range.startColumn - 1 + 3])

                let label = `vec${dim}(${color.red.toFixed(3)}, ${color.green.toFixed(3)}, ${color.blue.toFixed(3)}${dim === 4 ? `, ${color.alpha.toFixed(3)}` : ''})`
    ...
    editorInstance.addAction({
        ...
        run() {
            ...
            for (let [id, { view: { domNode } }] of currentZones) {
        ...
        editorInstance.addAction({
            ...
            run() {
                ...
                if (/^[ui]?vec[2-4]$/.test(wordObj.word)) {
                    ...
                    let components = value.trim().split(/ *, */).map(c => parseFloat(c));
                            ...
                            import('tweakpane').then(({ Pane }) => {
                                ...
                                let input = pane.addInput(
                                    ...
                                    'vec',
                                    {
                                        ...
                                        let text = Object.values(value).map(v => type ? v : parseFloat(v.toFixed(3))).join(', ');
                                        let range = Range.fromPositions(valueStartPos, valueStartPos.delta(0, prevValue.length));
                                        ...
                                        if (last) {
                                            ...
                                            model.pushEditOperations(
                                                null,
                                                [{
                                                    range: Range.fromPositions(valueStartPos, valueStartPos.delta(0, initialValue.length)),
                                                    text
                                                }],
                                                // @ts-ignore @todo
                                                null
                    ...
                    if (/^-?(?:(?:\d+\.\d*|\.\d+)(?:[eE][+-]?\d+)?|\d+[eE][+-]?\d+)[fF]?$/.test(wordObj.word)) {
                        // float
                        ...
                        messageController.showMessage('You can only tweak numerical values or constant vectors', position);
        ...
        font-weight: normal;
                            */
                            ...
                            import('tweakpane').then(({ Pane }) => {
                                ...
                                let input = pane.addInput({ number }, 'number', { step }).on('change', ({ last, value }) => {
        ...
        resolveRenameLocation(model, position, token) {
            ...
            if ('type' in def && def.qualifiers) {
                if (def.qualifiers.includes('uniform')) {
                    ...
                    return {
                        rejectReason: "You cannot rename input bindings directly. But it is possible during export"
                    }
                }
            }
            ...
            return {
                text: wordObj.word,
                range: new Range(position.lineNumber, wordObj.startColumn, position.lineNumber, wordObj.endColumn),
            }
        },
...

    {
        ...
        const buildFunctionDefinitionFromPrototypeNode = ({ header, parameters }: FunctionPrototypeNode) => {
            ...
            parameters?.forEach((p: ParameterDeclarationNode, i) => {
                ...
                let type = extractTypeString(p.declaration.type === 'parameter_declarator' ? (p.declaration.specifier as TypeSpecifierNode).specifier : p.declaration.specifier);
                ...
                let identifier = (p.declaration as ParameterDeclaratorNode)?.identifier?.identifier ?? 'arg_' + i;
            ...
            let documentation = (`${returnType} ${header.name.identifier}(${[...params.entries()].map(([name, [qualifiers, type]]) => `${qualifiers.length ? (qualifiers.join(' ') + ' ') : ''}${type} ${name}`).join(', ')});`)
         ...
         */
        const compareFunctionOverloads = (a: FunctionDefinitionParamsType, b: FunctionDefinitionParamsType) => {
            ...
            try {
                ...
                for (let scope of globalScopeList)
                    for (let [name, { references }] of scope.functions) {
                        ...
                        for (let node of nodes) {
                            ...
                            let [overloadDef, documentation] = buildFunctionDefinitionFromPrototypeNode(node.prototype)
                            ...
                            if (!dynamicFunctionDefinitionsData.has(name)) {
                                dynamicFunctionDefinitionsData.set(name, {
                                    ...
                                    range: Range.fromPositions({ lineNumber: scope.startLineNumber!, column: 1 }, fullModelEndPosition),
                    ...
                    references_label: for (let r of references) {
                        ...
                        */
                        if (r.type === 'function_prototype' || r.type === 'function') {
                            ...
                            let [overloadDef, documentation] = buildFunctionDefinitionFromPrototypeNode(prototype)
                ...
                for (let scope of globalScopeList) {
                    ...
                    for (let [name, binding] of bindings) {
                        ...
                        if (initializer.type === 'declaration') {
                            let qualifiers = initializer.specified_type.qualifiers?.map((q: KeywordNode) => q.token) ?? [];
                            ...
                            let def = {
                                ...
                                documentation: {
                                    value: glslMdWrap(qualifiers.length ? `${qualifiers.join(' ')} ${type} ${name}` : `${type} ${name}`) + docs
                                },
                                ...
                                sortText: (qualifiers.some(q => ['uniform', 'in', 'out'].includes(q)) ? AutocompletionOrder.UniformsInOut : AutocompletionOrder.VarConst).toString()
                ...
                for (let scope of editorAst.scopes.slice(1)) {
                    ...
                    for (let [name, binding] of bindings) {
                        ...
                        if (initializer.type === 'declaration' || initializer.type === "parameter_declaration") {
                            let type: string
                            ...
                            if (initializer.type === 'declaration') {
                                ...
                                // @note it can be null
                                qualifiers = initializer.specified_type.qualifiers?.map((q: KeywordNode) => q.token) ?? [];
                    ...
                    for (let [name, { references }] of types) {
                        ...
                        let { location: { start: structLocation } } = structNode
